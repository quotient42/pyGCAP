...
def count_blastp_result(project_info):
    ...
    target_df = pd.read_csv(f"{project_info['data']}/target.tsv", sep='\t')
    assembly_df = pd.read_csv(f"{project_info['output']}/tsv/assembly_report_sum.tsv", sep='\t')

    target = target_df['name'].values.tolist()
    target2 = target_df['prediction'].values.tolist()

    name = assembly_df['Name'].values.tolist() 
...
#===============================================================================
def count_mmseq_result(project_info):
